...
public class ImageSFXs {
	...
	public void drawResizedImage(Graphics2D g2d, BufferedImage im, int x,
			int y, double widthChange, double heightChange) {
	...
	public void drawVerticalFlip(Graphics2D g2d, BufferedImage im, int x, int y) {
		...
		int imWidth = im.getWidth();
		int imHeight = im.getHeight();
		g2d.drawImage(im, x + imWidth, y, x, y + imHeight, 0, 0, imWidth,
				imHeight, null);
	...
	public void drawHorizFlip(Graphics2D g2d, BufferedImage im, int x, int y) {
		...
		int imWidth = im.getWidth();
		int imHeight = im.getHeight();
		g2d.drawImage(im, x, y + imHeight, x + imWidth, y, 0, 0, imWidth,
				imHeight, null);
	...
	/*
	 * There are two versions of drawRedderImage() here: one using a LookupOp,
	 * the other a RescaleOp. There is functionally no difference between them.
	 */
	 ...
	 */
	public void drawRedderImage(Graphics2D g2d, BufferedImage im, int x, int y,
			float brightness) {
		if (im == null) {
	...
	/*
	 * public void drawRedderImage(Graphics2D g2d, BufferedImage im, int x, int
	 * y, float brightness) // using RescaleOp // Draw the image with its
	 * redness is increased, and its greenness // and blueness decreased. Any
	 * alpha channel is left unchanged. { if (im == null) {
	 * System.out.println("drawRedderImage: input image is null"); return; }
	 * 
	 * if (brightness < 0.0f) {
	 * System.out.println("Brightness must be >= 0.0f; setting to 0.0f");
	 * brightness = 0.0f; } // brightness may be less than 1.0 to make the image
	 * less red
	 * 
	 * RescaleOp brigherOp; if (hasAlpha(im)) { float[] scaleFactors =
	 * {brightness, 1.0f/brightness, 1.0f/brightness, 1.0f}; // don't change
	 * alpha // without the 1.0f the RescaleOp fails, which is a bug (?) float[]
	 * offsets = {64.0f, 0.0f, 0.0f, 0.0f}; brigherOp = new
	 * RescaleOp(scaleFactors, offsets, null); } else { // not transparent
	 * float[] scaleFactors = {brightness, 1.0f/brightness, 1.0f/brightness};
	 * float[] offsets = {64.0f, 0.0f, 0.0f}; brigherOp = new
	 * RescaleOp(scaleFactors, offsets, null); } g2d.drawImage(im, brigherOp, x,
	 * y); } // end of drawRedderImage() using RescaleOp
	 */

	// --------------- RescaleOp effects: brighten, negate ---------------
	/**
	 * Draw the image with changed brightness, by using a RescaleOp. Any alpha
	 * channel is unaffected.
	 */
	public void drawBrighterImage(Graphics2D g2d, BufferedImage im, int x,
			int y, float brightness) {
		if (im == null) {
	...
	/**
	 * Draw the image with 255-<colour value> applied to its RGB components. Any
	 * alpha channel is unaffected.
	 */
	...
	public void drawMixedColouredImage(Graphics2D g2d, BufferedImage im, int x,
			int y) {
		g2d.drawImage(getMixedColouredImage(im), x, y, null);
	} // end of drawMixedColouredImage()

	
	public BufferedImage getMixedColouredImage(BufferedImage im) {
		if (im == null) {
		...
		return new BufferedImage(im.getColorModel(), destRaster,
				false, null);

	} // end of getMixedColouredImage()
	// ------------------- Pixel effects -------------------
	// erasing, zapping

	/**
	 * Change some of the image's pixels to 0's, which will become transparent
	 * in an image with an alpha channel, black otherwise.
	 */
	...
	public void zapImageParts(BufferedImage im, double likelihood) {
		...
		for (int i = 0; i < pixels.length; i++) {
			rnd = Math.random();
			if (rnd <= likelihood) {
				if (rnd <= 15 * likelihood / 16) // make red more likely then
					// yellow
	...
	public BufferedImage copyImage(BufferedImage src) {
		...
		int transparency = src.getColorModel().getTransparency();
		BufferedImage copy = gc.createCompatibleImage(src.getWidth(), src
				.getHeight(), transparency);
		Graphics2D g2d = copy.createGraphics();
